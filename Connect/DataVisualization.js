...
function createDataTable(jsonData) {
    ...
    for(var i=0; i<parsedData.length; i++) {
        var productName = parsedData[i]["userfullname"];
        var zipCode = parsedData[i]["userpin"];
    ...
    for(var i=0; i<zipCodeArr.length; i++) {
        ...
        for(var j=0; j<productNameArr.length; j++) {
            for(k=0; k<parsedData.length; k++) {
                if(parsedData[k]["userpin"] === zipCodeArr[i] && parsedData[k]["userfullname"] === productNameArr[j]) {                    
