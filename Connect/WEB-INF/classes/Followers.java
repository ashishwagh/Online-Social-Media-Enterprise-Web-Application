...
public class Followers extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
		     follow = MySQLDataStoreUtilities.getFollowers(username);
		}
		catch(Exception e)
		{
			e.printStackTrace();
		...
		for(Map.Entry<String, Users> entry : follow.entrySet())
		{
			...
			pw.print("<li><form method='post' action='RemoveFollower'>" +
					...
					"<input type='submit' class='lbutton' value='Remove'></form></li>");
			...
			pw.print("<h3 style=\"color:green\">There are no Followers for you.</h3>");
		}
		pw.println("</section>");
